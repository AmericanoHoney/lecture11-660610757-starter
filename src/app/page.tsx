...
export default function RegisterForm() {
  ...
  const checkboxBottleOnChange = () => {
    setBuyBottle(!buyBottle);
  };

  const checkboxShoesOnChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setBuyShoes(event.target.checked);
  };

  const checkboxCapOnChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setBuyCap(event.target.checked);
  };

  const computeTotalPayment = () => {
    let totalPayment = 0;
    if (plan === "funrun") totalPayment +=500;
    else if (plan === "mini") totalPayment +=800;
    else if (plan === "half") totalPayment +=1200;
    else if (plan === "full") totalPayment +=1500;

    if (buyBottle) totalPayment +=200;
    if (buyShoes) totalPayment +=600;
    if (buyCap) totalPayment +=400;
  ...
  return (
    <div className="mx-auto vstack gap-3" style={{ width: "400px" }}>
      ...
      <div>
        ...
        <div>
          <input className="me-2 form-check-input" type="radio" onChange={radioGenderMaleOnChange} checked={gender == "male"} />
          Male 👨
          <input className="mx-2 form-check-input" type="radio" onChange={radioGenderFemaleOnChange} checked={gender == "female"}/>
          Female 👩
        </div>
      </div>
      {gender}
      ...
      <div>
        ...
        <div>
          <input className="form-check-input" type="checkbox" onChange={checkboxBottleOnChange}/>{" "}
          <label className="form-check-label">Bottle 🍼 (200 THB)</label>
        </div>
        <div>
          <input className="form-check-input" type="checkbox" onChange={checkboxShoesOnChange}/>{" "}
          <label className="form-check-label">Shoes 👟 (600 THB)</label>
        </div>
        <div>
          <input className="form-check-input" type="checkbox" onChange={checkboxCapOnChange}/>{" "}
